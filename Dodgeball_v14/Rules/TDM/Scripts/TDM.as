...
shared class TDMCore : RulesCore
{
	...
	{
		TDMPlayerInfo p(player.getUsername(), player.getTeamNum(), player.isBot() ? "archer" : (XORRandom(512) >= 256 ? "archer" : "archer"));
		players.push_back(p);
	...
	void GiveSpawnResources(CBlob@ blob, CPlayer@ player)
	{
		...
		if (blob.getName() == "builder")
		{
			...
			CMap@ map = getMap();
			bool found = false;
			if (map.getBlobsInRadius(blob.getPosition(), 0.0f, @blobsInRadius))
			{
