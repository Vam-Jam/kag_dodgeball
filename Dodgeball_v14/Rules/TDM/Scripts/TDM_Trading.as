...
void MakeTradeMenu(CBlob@ trader)
{
	...
	s32 cost_balld = cfg.read_s32("cost_balld", 15);
	s32 cost_ballg = cfg.read_s32("cost_ballg", 50);
	s32 cost_balle = cfg.read_s32("cost_balle", 35);
	...
	if (cost_balld > 0)
		addItemForCoin(trader, "Normal Ball", cost_balld, true, "$balld$", "balld", "A normal ball.");

	if (cost_ballg > 0)
		addItemForCoin(trader, "God ball", cost_ballg, true, "$ballg$", "ballg", "A ball sent from god, It can team kill and kill others in an instant.");

	if (cost_balle > 0)
		addItemForCoin(trader, "Chatty ball", cost_balle, true, "$balle$", "balle", "A ball that chats on kill");
}
